...
export async function deleteLesson({
  ...
  await requireAdmin();
  try {
    const chapterWithLesssons = await prisma.chapter.findFirst({
      where: {
        id: chapterId,
        courseId,
      },
    ...
    const lessons = chapterWithLesssons.lessons;
...
export async function deleteChapter({
  ...
  try {
    ...
    const remainingChapters = chapters.filter(
      (chapter) => chapter.id !== chapterToDelete.id
    );
